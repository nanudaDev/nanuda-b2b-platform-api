...
export class CompanyDistrictAnalysisService extends BaseService {
  ...
  async avgStoreSales(param) {
    ...
    const query = `(WITH cte AS (SELECT t5.kb_id, COUNT(DISTINCT t5.trans_dt) AS dt_count, SUM(t5.trans_amt) AS trans_amt \
      ...
      FROM (SELECT ${lat} AS latpoint, ${lon} AS longpoint, ${radius +
      100} AS r, 111045 AS units) AS t3) AS t2 \
      ...
      FROM wq.kb_trans_data) GROUP BY 1,2 having distance < ${radius}) as t5 GROUP BY 1 ) SELECT avg(trans_amt/dt_count/3)/0.16 AS KB_AVG_AMT FROM cte) ;`;
  ...
  async avgBizSales(param) {
    ...
    FROM wq.kb_trans_data) GROUP BY 1,2 having distance < ${radius}) as t5 \
  ...
  async ageGroupDis(param) {
    ...
    FROM (SELECT ${lat} AS latpoint, ${lon} AS longpoint, ${radius +
      100} AS r, 111045 AS units) AS t3) AS t2 ON (1=1) WHERE t1.lat BETWEEN lat_min AND lat_max AND t1.lon \
    BETWEEN lon_min AND lon_max AND t1.storetype_cd \
    IN (2004, 2099, 2104, 2107, 2109, 2110, 2111, 2112, 2113, 2199) \
    GROUP BY 1 having distance < ${radius} order by distance asc) SELECT SUM(A10)/SUM(TOTAL) AS A10, SUM(A20)/SUM(TOTAL) AS A20, SUM(A30)/SUM(TOTAL) AS A30, SUM(A40)/SUM(TOTAL) AS A40, SUM(A50)/SUM(TOTAL) AS A50, SUM(A60)/SUM(TOTAL) AS A60 FROM cte) ;`;
  ...
  async dayDistribution(param) {
    ...
    ON (1=1) WHERE t1.lat BETWEEN lat_min AND lat_max AND t1.lon BETWEEN lon_min AND lon_max AND t1.storetype_cd \
    IN (2004, 2099, 2104, 2107, 2109, 2110, 2111, 2112, 2113, 2199) \
    GROUP BY 1 having distance < ${radius} order by distance asc) SELECT SUM(MON)/SUM(TOTAL) AS MON, SUM(TUE)/SUM(TOTAL) AS TUE, SUM(WED)/SUM(TOTAL) AS WED, SUM(THU)/SUM(TOTAL) AS THU, SUM(FRI)/SUM(TOTAL) AS FRI, SUM(SAT)/SUM(TOTAL) AS SAT, SUM(SUN)/SUM(TOTAL) AS SUN FROM cte) ;`;
  ...
  async genderDistribution(param) {
    ...
    BETWEEN lon_min AND lon_max AND t1.storetype_cd \
    IN (2004, 2099, 2104, 2107, 2109, 2110, 2111, 2112, 2113, 2199) GROUP BY 1 having distance < ${radius} order by distance asc) SELECT SUM(M)/SUM(M + FM) AS M, SUM(FM)/SUM(M + FM) AS FM FROM cte) ;`;
  ...
  async employeeRatio(param) {
    ...
    IN (SELECT MAX(data_crt_ym) AS data_crt_ym FROM wq.nps_data) having distance < ${radius}) UNION ALL (SELECT t1.pk_id, t1.jnngp_cnt, t1.crrmm_ntc_amt, ${radius} as distance \
    FROM wq.nps_data t1 \
