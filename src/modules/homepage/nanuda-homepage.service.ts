...
export class NanudaHomepageService extends BaseService {
    ...
    async findBestSpaces() {
        const response = []
        const bestRestaurantKitchen = await this.entityManager.getRepository(BestSpaceMapper)
        .createQueryBuilder('bestSpace')
        .CustomInnerJoinAndSelect(['space'])
        .where('bestSpace.spaceTypeNo = :spaceTypeNo', {spaceTypeNo: SPACE_TYPE.SPACE_SHARE})
        ...
        await Promise.all(
            bestRestaurantKitchen.map(async space => {
                const image = await this.entityManager.getRepository(FileManagement).find({
                    where: {
                        targetTable: 'SPACE', 
        ...
        const bestDeliverySpace = await this.entityManager.getRepository(BestSpaceMapper)
        .createQueryBuilder('bestSpace')
        .CustomInnerJoinAndSelect(['deliverySpace'])
        .where('bestSpace.spaceTypeNo = :spaceTypeNo', {spaceTypeNo: SPACE_TYPE.ONLY_DELIVERY})
